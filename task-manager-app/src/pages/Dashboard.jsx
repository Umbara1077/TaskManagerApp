...
const Dashboard = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await createTask({
      title, description, priority, dueDate, dueTime, status: 'Pending', createdBy: currentUser.uid
    });
      ...
      <ul>
        {tasks.map((task) => (
          <li key={task.id} className="task-item">
            <strong>{task.title}</strong> - {task.priority} - {task.status}<br />
            Due: {new Date(task.dueDate.seconds * 1000).toLocaleDateString()} at {new Date(task.dueDate.seconds * 1000).toLocaleTimeString()}
      ...
      <ul>
        {completedTasks.map((task) => (
          <li key={task.id} className="task-item completed">
            <strong>{task.title}</strong> - {task.priority} - ✅ Completed<br />
            Completed on: {new Date(task.dueDate.seconds * 1000).toLocaleDateString()} at {new Date(task.dueDate.seconds * 1000).toLocaleTimeString()}
          </li>
        ))}
      </ul>
